...
- `laser`: armed, hit_detected 상태

Jetson에서 `laser.hit_detected=true`를 보내면 Pi의 드론 제어가 `flip_forward` 뒤 `land`를 실행할 수 있습니다. 이 동작은 `python -m tello_control.cli --auto-hit-response`로 켭니다.
